...
#define FREQUENCY (1008-42)

#define N_INTEGRA_TIME 10       // N_INTEGRA_TIME integration times in one buf
#define buflen ((MAX_PACKET_ID-1)*MAX_PACKET_SIZE + MIN_PACKET_SIZE )*FREQUENCY*N_INTEGRA_TIME 
...
void gen_datafile()
{
    ...
    sprintf(filename, "lz1_%05d.dat", file_count);
    strcat(data_path0,filename);

    fp_data = fopen(data_path0,"wb");
    file_count++;
}
...
{       
    ...
    while (1)
    {
        if( buf01_state == 1 )
        {  
            fwrite(buf01, sizeof(unsigned char), buflen, fp_data);
            fclose(fp_data);
        ...
        {
            fwrite(buf02, sizeof(unsigned char), buflen, fp_data);
            fclose(fp_data);
            for (buf_init_iter=0; buf_init_iter<buflen; buf_init_iter++)
...
void recvData()
{
    ...
    register long row_size = ((MAX_PACKET_ID - 1) * MAX_PACKET_SIZE + MIN_PACKET_SIZE);
...
//        if (packet_len == MAX_RAWPACKET_SIZE || packet_len == MIN_RAWPACKET_SIZE)
    ...
    while(Running)
    {
        ...
        if (buf01_state == 0)
        {
            ...
            {
                memcpy(buf01 + row*row_size + pkt_id*MAX_PACKET_SIZE, frame_buff_p + 22, packet_len - 22);
//                while (1)
//                {
                    packet_len = recv(recv_fd, frame_buff, BUFSIZE, 0);
//                    if (packet_len == MAX_RAWPACKET_SIZE || packet_len == MIN_RAWPACKET_SIZE)
...
//                }
        ...
        else if (buf02_state == 0)
        {
            while (row < row_in_buf)
            {
                memcpy(buf02 + row*row_size + pkt_id*MAX_PACKET_SIZE, frame_buff_p + 22, packet_len - 22);
//                while (1)
//                {
                    packet_len = recv(recv_fd, frame_buff, BUFSIZE, 0);
//                    if (packet_len == MAX_RAWPACKET_SIZE || packet_len == MIN_RAWPACKET_SIZE)
